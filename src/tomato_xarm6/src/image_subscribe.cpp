...
namespace tomato_xarm6 {
    ImageSubscriber::ImageSubscriber(const std::string &node_name, double camera_FOV, int width, int height, bool capture_both, std::string &topic_name, bool reduce_file_size) 
    {
        ...
        sync_sub_segment_ = std::make_unique<message_filters::Subscriber<sensor_msgs::msg::Image>>();
    ...
    void ImageSubscriber::RGBDImageCallback(const sensor_msgs::msg::Image::ConstSharedPtr& msg_color, const sensor_msgs::msg::Image::ConstSharedPtr& msg_segment, const sensor_msgs::msg::Image::ConstSharedPtr& msg_depth)
    {
        ...
        RCLCPP_DEBUG(node_->get_logger(), "RGBDImageCallback");
    ...
    {
        RCLCPP_DEBUG(node_->get_logger(), "StereoImageCallback");
    ...
    {
        RCLCPP_DEBUG(node_->get_logger(), "clearing image topics");
        ...
        sync_sub_color_->subscribe(node_, "/ue5/"+topic_name_+"/Color");
        sync_sub_color1_->subscribe(node_, "/ue5/"+topic_name_+"/ColorOne");
        sync_sub_segment_->subscribe(node_, "/ue5/"+topic_name_+"/Segmentation");
        sync_sub_depth_->subscribe(node_, "/ue5/"+topic_name_+"/Depth");
        sync_sub_depth1_->subscribe(node_, "/ue5/"+topic_name_+"/DepthOne");
        RCLCPP_DEBUG(node_->get_logger(), "clearing stereo topics");
        stereo_sync_ = std::make_shared<message_filters::Synchronizer<ApproxTimeSyncPolicyStereo>>(
            ApproxTimeSyncPolicyStereo(1),
            *sync_sub_color_, *sync_sub_color1_, *sync_sub_depth_, *sync_sub_depth1_
        );
        //stereo_sync_->setMaxIntervalDuration(rclcpp::Duration::from_seconds(0.1));
        stereo_sync_->registerCallback(
            std::bind(&ImageSubscriber::StereoImageCallback, this, 
                std::placeholders::_1, std::placeholders::_2, std::placeholders::_3,std::placeholders::_4));

        RCLCPP_DEBUG(node_->get_logger(), "clearing rgbd topics");
        rgbd_sync_ = std::make_shared<message_filters::Synchronizer<ApproxTimeSyncPolicyRGBD>>(
            ApproxTimeSyncPolicyRGBD(1),
            *sync_sub_color_, *sync_sub_segment_, *sync_sub_depth_
        );
        //rgbd_sync_->setMaxIntervalDuration(rclcpp::Duration::from_seconds(0.1));
        rgbd_sync_->registerCallback(
            std::bind(&ImageSubscriber::RGBDImageCallback, this, 
                std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
        waiting_msg_rgbd = true;
    ...
    void ImageSubscriber::waiting_for_sync()
    {
        ...
        while((capture_both_ && (waiting_msg_rgbd || waiting_msg_stereo)) ||
            ...
            RCLCPP_DEBUG(node_->get_logger(), "waiting for sync - Image");
            lock.unlock();
    ...
    void ImageSubscriber::save_images(std::string path)
    {
        ...
        if (!cv_img_.empty()){
            if (reduce_file_size_) {
                ...
                cv::imwrite(path + "/rgb_" + std::to_string(capture_count_) + ".jpg", cv_img_, compression_params);
            } else {
        ...
        if (!cv_img1_.empty()){
            if (reduce_file_size_) {
                ...
                compression_params.push_back(cv::IMWRITE_JPEG_QUALITY);
                compression_params.push_back(95);
                cv::imwrite(path + "/rgb1_" + std::to_string(capture_count_) + ".jpg", cv_img1_, compression_params);
            } else {
        ...
        if (!cv_img_depth_.empty()){
            if (reduce_file_size_) {
                cv::imwrite(path + "/depth_" + std::to_string(capture_count_) + ".png", cv_img_depth_);
            }
            else {
        ...
        if (!cv_img1_depth_.empty()){
            if (reduce_file_size_) {
                cv::imwrite(path + "/depth1_" + std::to_string(capture_count_) + ".png", cv_img1_depth_);
            }
            else {
